...
工作台内置口播常用组件和 SFX cue。组件默认尽量透明，方便统一放在每个镜头背景上。列表类组件支持单独调整标题字号、条目字号、条目背景、条目文字色和局部偏移；文字类组件支持标题行与下行文字分别设置字号、字重、字体和颜色。组件会逐步向“预设组件组 + 可编辑子层”的方向演进，避免死模板带来的微调不灵活。
